...
/**
 * "args": ["-n","first difference","testcopy2.c"],
 */
...
void fpat(FILE *fp, char *fname, char *pattern, int except, int number)
{
    ...
    {
        ++lineno;
        if ( (strstr(line, pattern) != NULL) != except) //finds the first occurrence of the substring [pattern] in the string [line]
        {
